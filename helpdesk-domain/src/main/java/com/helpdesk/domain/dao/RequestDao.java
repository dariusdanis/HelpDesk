...
public interface RequestDao {
	...
	List<RequestEntity> getAllByEngineerAndStatus(UserEntity engineerEntity,
			String status, int from);

	List<RequestEntity> getAllByStatus(String status, int from);

	List<RequestEntity> getAllByEngineerAndNotStatus(UserEntity engineerEntity,
			String status, int from);

	List<Integer> getTopThree();

	List<RequestEntity> getAllByAdminAndNotStatus(UserEntity user,
			String status, int from);

	List<RequestEntity> getAllByBelongsTosAndNotStatu(UserEntity belongsTo,
			String status, int from, String status2);

	List<RequestEntity> getAllByBelongsToAndStatus(UserEntity belongsTo,
			String status, String status2, int from);

	List<RequestEntity> getAllByStatusOrAssignetToUser(String status,
			UserEntity user, int from);

	List<RequestEntity> getDirectorHistory(UserEntity director, String solved,
			String assigned, int from);
	...
	Long getAllByBelongsTosAndNotStatuCount(UserEntity belongsTo,
	...
	Long getDirectorHistoryCount(UserEntity director, String solved,
			String assigned);

	Long getTodaySolvedCount(Date today);

	Long getTodayNewReq(Date today);

	Long getTodayNewInfoReq(Date today);
}
